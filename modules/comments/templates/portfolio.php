...
<div class="mwcommentsmodule mwcommentsmodule-portfolio ">
  ...
  ?>
    ...
    <hr>
    <script>
        ShowPortfolioCommentForm = function(){
            mw.$('#show_comment_form_btn').hide().next().show();
            var holder = mw.$('#portfolio-inner-content');
            holder.animate({ scrollTop:holder[0].scrollHeight });
        }
    </script>
    <a href="javascript: ;" onclick="ShowPortfolioCommentForm();" id="show_comment_form_btn" class="in-btn">Leave a comment</a>
    <div class="portfolio-comment-form-holder" id="portfolio-comment-form-holder">
    ...
    <div class="mw-comments-form" id="comments-<?php print $data['id'] ?>">
      ...
      <form autocomplete="on" id="comments-form-<?php print $data['id'] ?>" class="form-group">
        ...
        <div class="mw-ui-row vertical-middle captcha-row">
          ...
          <div class="mw-ui-col"> <span onclick="mw.tools.refresh_image(mwd.getElementById('comment-captcha-<?php print $rand; ?>'));" class="mw-icon-irefresh"></span> </div>
        </div>
